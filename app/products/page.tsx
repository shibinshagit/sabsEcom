...
export default function ProductsPage() {
  ...
  useEffect(() => {
    // Show spinner for unauthenticated users, once per session
    if (!isAuthenticated) {
      const hasSeenSpinner = sessionStorage.getItem("hasSeenOfferSpinner")
      
      if (!hasSeenSpinner) {
        // Show spinner after a short delay for better UX
      ...
      // If user logs in, check for pending offers
      const pendingOffer = localStorage.getItem("pendingOffer")
      if (pendingOffer) {
        try {
          const offerData = JSON.parse(pendingOffer)
          // You can show a success message or apply the discount here
          console.log("User has pending offer:", offerData)
          
          // Optional: Show a notification about the available discount
          // toast.success(`Welcome back! Your ${offerData.title} discount code ${offerData.code} is ready to use!`)
          
          // Keep the offer in localStorage until used
        } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        {/* Show Spin Button for unauthenticated users */}
        {!isAuthenticated && !showSpinner && (
          <div className="mb-8 text-center">
            <Button
              onClick={() => setShowSpinner(true)}
              className="bg-gradient-to-r from-purple-500 to-pink-600 hover:from-purple-600 hover:to-pink-700 text-white px-8 py-3 rounded-full font-bold text-lg shadow-lg hover:shadow-xl transition-all duration-300 animate-pulse"
            >
              🎯 Spin for Discount! 
            </Button>
            <p className="text-sm text-gray-600 mt-2">Get up to 100% OFF on your first order!</p>
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredItems.map((item, index) => (
              <Card
                key={item.id}
                className={`group hover:shadow-2xl transition-all duration-300 animate-slide-up rounded-2xl border-0 bg-white/70 backdrop-blur-xl ${theme.shadow} relative overflow-hidden`}
                style={{ animationDelay: `${index * 0.05}s` }}
              >
                {/* Premium border overlay */}
                <div className={`absolute inset-0 pointer-events-none rounded-2xl border-2 ${shop === "A" ? "border-yellow-300/60" : "border-gray-300/60"} z-10`} style={{ boxShadow: shop === "A" ? '0 0 0 2px #ffe06655' : '0 0 0 2px #bfc1c655' }} />
