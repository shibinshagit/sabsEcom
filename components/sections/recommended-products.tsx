      ...
      <div className="bg-background py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-foreground mb-4">Recommended Products</h2>
            <p className="text-muted-foreground">Loading similar products...</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[...Array(4)].map((_, index) => (
              <div key={index} className="animate-pulse">
                <div className="bg-muted aspect-square mb-4"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden sm:grid sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {recommendedProducts.slice(0, 5).map((product) => {
            ...
            return (
              <Card 
                key={product.id} 
                className="group cursor-pointer hover:shadow-md transition-all duration-300 transform hover:-translate-y-1 bg-card border border-border shadow-sm overflow-hidden"
                ...
                <CardContent className="p-0">
                  ...
                  <div className="relative aspect-square overflow-hidden bg-muted">
                    ...
                    <div className="absolute top-3 left-3 flex flex-col gap-2">
                      ...
                      {product.is_featured && (
                        <Badge className="bg-muted text-foreground text-xs px-2 py-1">
                          Featured
                        </Badge>
                      )}
                      {hasDiscount && (
                  ...
                  <div className="p-3">
                    <div className="mb-2">
                      <h3 className="font-medium text-sm text-foreground line-clamp-2 transition-colors">
                        {product.name}
                      </h3>
                      {product.brand && (
            ...
            {recommendedProducts.slice(0, 6).map((product) => {
              ...
              return (
                <div 
                  ...
                  onClick={() => handleProductClick(product.id)}
                >
                  <Card className="group hover:shadow-md transition-all duration-300 bg-card border border-border shadow-sm overflow-hidden h-full">
                    <CardContent className="p-0">
                      {/* Product Image */}
                      <div className="relative aspect-square overflow-hidden bg-muted">
                        <Image
                        ...
                        <div className="absolute top-2 left-2 flex flex-col gap-1">
                          {product.is_new && (
                            <Badge className="bg-foreground text-background text-xs px-1.5 py-0.5">
                              New
                            </Badge>
                          )}
                          {product.is_featured && (
                            <Badge className="bg-muted text-foreground text-xs px-1.5 py-0.5">
                              Featured
                            </Badge>
                          )}
                          {hasDiscount && (
                      ...
                      <div className="p-2">
                        <div className="mb-1">
                          <h3 className="font-medium text-xs text-foreground line-clamp-2 transition-colors">
                            {product.name}
                          </h3>
                        </div>
                        ...
                        {/* Category */}
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 truncate">
                            {product.category_name}
                          </span>
        ...
        <div className="text-center mt-8">
          <Button
            ...
            variant="outline"
            className="px-6 py-2 text-sm text-foreground border-border hover:bg-muted"
          >
