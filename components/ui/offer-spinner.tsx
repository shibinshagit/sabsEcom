...
const SpinWheel = ({ onClose }: SpinWheelProps) => {
  ...
  // Color options for different segments
  const colors = ["gold", "purple", "blue", "green"]
    ...
    const fetchOffers = async () => {
      try {
        ...
        if (activeOffer) {
          ...
          // Convert to wheel format with different colors for each offer
          const wheelOffers: WheelOffer[] = offerDiscounts.map((discount, index) => ({
            id: index + 1,
            title: `${discount.percentage}% OFF`,
            discount: discount.percentage,
            color: colors[index % colors.length] as "gold" | "purple" | "blue" | "green"
          }))
  ...
  const generateWheelBackground = () => {
    ...
    const gradientStops = wheelOffers.map((offer, index) => {
      ...
      const color = getColorValue(offer.color)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="relative mb-6">
        ...
        {/* Main wheel */}
        <div className="relative w-44 h-44 rounded-full border-4 border-yellow-400 shadow-2xl overflow-hidden">
          <div
            ref={wheelRef}
          ...
          >
            ...
            {wheelOffers.map((offer, index) => {
              ...
              return (
                ...
                >
                  <div
                    className="absolute text-white font-bold text-sm transform -translate-y-12"
                    style={{ 
